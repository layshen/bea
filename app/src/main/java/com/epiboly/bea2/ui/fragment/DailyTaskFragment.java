...
public class DailyTaskFragment extends TitleBarFragment<HomeMainActivity> implements StatusAction {
    ...
    private QueryProductApi.Bean bean;
...
//        });
    ...
    private void queryUserFinishStatusApi() {
        EasyHttp.post(this)
                ...
                .api(new QueryProductApi().setToken(UserHelper.getInstance().getToken()))
                .request(new HttpCallback<HttpData<QueryProductApi.Bean>>(this) {
                    @Override
                    public void onSucceed(HttpData<QueryProductApi.Bean> result) {
...
//        }
    ...
    private void setupContent(QueryProductApi.Bean bean) {
        this.bean = new QueryProductApi.Bean();
        bean = this.bean;
        bean.setFinishedCount(0);
        bean.setTotalCount(1000);
